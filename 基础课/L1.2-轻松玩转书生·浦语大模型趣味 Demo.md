...
```bash
/root/share/install_conda_env_internlm_base.sh xcomposer-demo2
conda activate xcomposer-demo2
pip install transformers==4.33.1 timm==0.4.12 sentencepiece==0.1.99 gradio==3.44.4 markdown2==2.4.10 xlsxwriter==3.1.2 einops accelerate
```
![alt text](img2/进阶任务-环境准备.png)
### 5.2 模型下载
在[魔搭社区](https://www.modelscope.cn/models)根据模型名称查找模型[Shanghai_AI_Laboratory/internlm-xcomposer2-vl-1_8b](https://www.modelscope.cn/models/Shanghai_AI_Laboratory/internlm-xcomposer2-vl-1_8b)，在 /root/code 目录下新建download_model_by_modelscope.py 文件并在其中输入以下内容
```python
import torch
from modelscope import snapshot_download, AutoModel, AutoTokenizer
import os
model_dir = snapshot_download('Shanghai_AI_Laboratory/internlm-xcomposer2-vl-1_8b', cache_dir='/root/model', revision='master')
```
运行 python download_model_by_modelscope.py 执行下载
...
cd /root/code/InternLM-XComposer
python examples/web_demo.py  \
    --folder /root/model/Shanghai_AI_Laboratory/internlm-xcomposer2-vl-1_8b \
    --num_gpus 1 \
    --port 6006
```
![alt text](img2/终端运行internlm-xcomposer2-vl-1_8b.png)
运行失败报错：
![alt text](img2/终端运行internlm-xcomposer2-vl-1_8b报错.png)

## 6 作业 - 进阶任务2
使用 LMDeploy 完成 InternVL2-2B 的部署，并完成一次图文理解对话，记录复现过程并截图。
### 6.1 进入xcomposer-demo环境
首先在 InternStudio 上创建并启动A100*30% 开发机。然后初始化conda环境：
```bash
conda activate xcomposer-demo
```
### 6.2 模型下载
在[魔搭社区](https://www.modelscope.cn/models)根据模型名称查找模型[OpenGVLab/InternVL2-2B](https://www.modelscope.cn/models/OpenGVLab/InternVL2-2B)，在 /root/code 目录下新建download_model_by_modelscope-token.py 文件并在其中输入以下内。容  
```python
#验证SDK token
from modelscope.hub.api import HubApi
api = HubApi()
api.login('xxxxxx') #魔搭平台获取token
#模型下载
from modelscope import snapshot_download
model_dir = snapshot_download('OpenGVLab/InternVL2-2B', cache_dir='/root/model')
```
运行 python download_model_by_modelscope-token.py 执行下载
![alt text](img2/从魔搭下载模型InternVL2-2B.png)
### 6.3 代码准备
```bash
cd /root/code
git clone https://gitee.com/internlm/InternLM-XComposer.git
...
cd /root/code/InternLM-XComposer
python examples/web_demo.py  \
    --folder /root/model/OpenGVLab/InternVL2-2B \
    --num_gpus 1 \
    --port 6006
```
运行失败报错，暂未找到原因：
![alt text](img2/终端运行InternVL2-2B报错.png)
